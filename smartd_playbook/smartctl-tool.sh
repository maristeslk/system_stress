 ...
 *Lenovo*)
        vendor="lenovo"
        raid_tool='storcli64'
        wget https://agora-devops-public-2.oss-cn-beijing.aliyuncs.com/Stress-testing/bin/perccli64 -O /usr/bin/storcli64
        chmod +x /usr/bin/storcli64
        ;;
 *Huawei*)
        vendor="huawei"
        raid_tool='storcli64'
        if command -v storcli64 >/dev/null 2>&1; then 
            echo 'exists storcli64' 
        else
            echo 'no exists storcli64' 
            wget https://agora-devops-public-2.oss-cn-beijing.aliyuncs.com/Stress-testing/bin/perccli64 -O /usr/bin/storcli64
            chmod +x /usr/bin/storcli64
        fi 
 ...
 *Dell*)
        ...
        if command -v perrccli64 >/dev/null 2>&1; then 
            echo 'exists perrccli64' 
        else
            echo 'no exists perrccli64' 
            wget https://agora-devops-public-2.oss-cn-beijing.aliyuncs.com/Stress-testing/bin/perccli64 -O /usr/bin/perrccli64
            chmod +x /usr/bin/perrccli64
        fi 
        ;;
 *HP*)
        vendor="hp"
...
if command -v smartctl >/dev/null 2>&1; then 
  echo 'exists smartctl' 
else 
  echo 'no exists smartctl' 
    apt-get -y install smartmontools  lsscsi --no-install-recommends
...
smartd_create() {
  ...
  if [ "$CONTROLL_TYPE" == "megaraid" ];then
        if command -v megaclisas-status >/dev/null 2>&1; then 
          :
        else 
          wget https://agora-devops-public-2.oss-cn-beijing.aliyuncs.com/Stress-testing/bin/megaclisas-status.py -O /usr/bin/megaclisas-status
  ...
  #CHART type.id         name    title                                    units [family [context [charttype [priority [update_every [options [plugin [module]]]]]]]]
            cat  << EOF
CHART  smartd.status_${DISK} '' "smart_info on ${DISK}" "smart" "${DISK}" "disk" smart_value smart_info_${DISK} line $((smartd_priority + 1)) $smartd_update_every
