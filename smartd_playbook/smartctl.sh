...
function install(){
    if  [ $system_version == '18.04' ];then
        ...
        add-apt-repository "deb [arch=amd64] http://hwraid.le-vert.net/ubuntu  $(lsb_release -cs)   main"
        apt-get update
        apt-get install -y megaclisas-status
        ...
        TYPE="cciss"
            ...
            for info  in $( cat /var/lib/smartmontools/disk_sn.txt )
                    do
                            ...
                            SLOT_NUM=$( lsscsi | grep $DISK | awk '{print $1}')
                            SLOT_NUM=${SLOT_NUM#"["}
                            SLOT_NUM=${SLOT_NUM%"]"}
